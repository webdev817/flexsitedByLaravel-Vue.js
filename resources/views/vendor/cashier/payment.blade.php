...
<body class="font-sans text-gray-600 bg-gray-200 leading-normal p-4 h-full">
    <div id="app" class="h-full md:flex md:justify-center md:items-center">
        <div class="w-full max-w-lg">
            <!-- Status Messages -->
            <p class="flex items-center mb-4 bg-red-100 border border-red-200 px-5 py-2  rounded-none text-red-500" v-if="errorMessage">
            ...
            <p class="flex items-center mb-4 bg-green-100 border border-green-200 px-5 py-4  rounded-none text-green-700" v-if="paymentProcessed && successMessage">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="flex-shrink-0 w-6 h-6">
            ...
            <div class="bg-white  rounded-none shadow-xl p-4 sm:py-6 sm:px-10 mb-5">
                ...
                @else
                    <div id="payment-elements" v-if="! paymentProcessed">
                        ...
                        <input id="cardholder-name" type="text" placeholder="{{ __('Jane Doe') }}" required
                               class="inline-block  border border-gray-400  rounded-none w-full px-4 py-3 mb-3 focus:outline-none"
                               v-model="name">

                        <!-- Card -->
                        <label for="card-element" class="inline-block text-sm text-gray-700 font-semibold mb-2">{{ __('Card') }}</label>

                        <div id="card-element" class="  border border-gray-400  rounded-none p-4 mb-6"></div>

                        <!-- Pay Button -->
                        <button id="card-button"
                                class="btnCustomize inline-block w-full px-4 py-3 mb-4 text-white rounded-none"
                ...
                <a href="{{ $redirect ?? url('/') }}"
                   class="inline-block w-full px-4 py-3 bg-gray-200 hover:bg-gray-300 text-center text-gray-700  rounded-none">
                    {{ __('Go back') }}
                </a>
            </div>
